...
const API_KEY = 'eyJhbGciOiJIUzI1NiJ9.eyJzdWIiOiJ6ZW5jYXJlNGtAZ21haWwuY29tIiwianRpIjoiZWYzNDgxZTktNGQ3NS00MjAzLWJjYjktYmI3YzY0ZmI5ODUwIiwiaXNzIjoiQUVNRVQiLCJpYXQiOjE3NDAzNDMwMTgsInVzZXJJZCI6ImVmMzQ4MWU5LTRkNzUtNDIwMy1iY2I5LWJiN2M2NGZiOTg1MCIsInJvbGUiOiIifQ.qhraO1f2kCNoaWWc7E7Vpc6oVmeu-g8boDP1-SCu-TE';
...
// Obtener municipios por latitud y longitud
export const getMunicipiosByLatLon = async (lat, lon) => {
  try {
    const response = await axios.get(`${BASE_URL}/maestro/municipio`, {
      params: { latitud: lat, longitud: lon, api_key: API_KEY },
    });

    if (response.data && response.data.datos) {
      const dataUrl = response.data.datos;
      const municipiosData = await axios.get(dataUrl);
      return municipiosData.data;
    }

    throw new Error('Respuesta inválida de la API de AEMET');
  } catch (error) {
    console.error('Error fetching municipios data:', error.message);
    throw error;
  }
};

// Obtener estaciones meteorológicas más cercanas
export const getNearbyWeatherStations = async (lat, lon) => {
  try {
    const response = await axios.get(`${BASE_URL}/observacion/convencional/todas`, {
      params: { api_key: API_KEY },
    });
    ...
    if (response.data && response.data.datos) {
      const dataUrl = response.data.datos;
      const weatherData = await axios.get(dataUrl);
      return weatherData.data.filter(item => {
        const distance = Math.sqrt(Math.pow(item.latitud - lat, 2) + Math.pow(item.longitud - lon, 2));
        return distance < 0.1; // Ajusta este valor según la proximidad deseada
      });
    }

    throw new Error('Respuesta inválida de la API de AEMET');
  } catch (error) {
    console.error('Error fetching nearby weather stations data:', error.message);
    throw error;
  }
};
