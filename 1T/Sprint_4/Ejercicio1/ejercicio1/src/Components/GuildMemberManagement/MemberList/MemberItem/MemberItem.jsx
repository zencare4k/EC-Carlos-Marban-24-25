...
import { updateGuildMember } from '../../../../Services/guildmembers_API';
...
const MemberItem = ({ member, isSelected, onToggleSelect, onDeleteMember }) => {
  ...
  const handleSaveEdit = async (updatedMember) => {
    try {
      ...
      // Optionally, update the local state or trigger a refresh of the member list
      setIsEditModalOpen(false);
    } catch (error) {
      setError('Error saving member changes');
    }
  };
